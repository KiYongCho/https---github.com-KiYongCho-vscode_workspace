...
console.log(arr1.map(ele=>+ele).filter(ele=>ele%3==0));
...
// substring(시작인덱스, 끝인덱스+1) : 문자열에서 시작인덱스부터 끝인덱스까지의 부분문자열
persons.forEach(
    ele => {
        console.log(
            ele.name + ',' +
...
students.sort((a, b) => {
    ...
    return (a.name>b.name) ? 1 : -1; // 이름 오름차순
});
...
// sort함수의 콜백함수가 양수를 반환(앞/뒤=>앞/뒤), 0을 반환(위치변경없음), 음수를 반환(앞/뒤=>뒤/앞)
obj.sort((a, b) => {
    if (a.age!=b.age) return b.age - a.age; // 나이가 다르면 나이 내림차순
    // 나이가 같으면 풀네임 내림차순
    else return b.name.lname+b.name.fname 
                        > a.name.lname+a.name.fname ? 1 : -1;
});
console.log(obj);
